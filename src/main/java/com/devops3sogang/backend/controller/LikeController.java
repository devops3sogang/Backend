...
public class LikeController {
     ...
     *
     * - 좋아요 생성 시: 201 Created
     * - 좋아요 취소 시: 200 OK
     */
    @PostMapping("/{reviewId}/like")
    @SecurityRequirement(name = "Bearer Authentication")
    public ResponseEntity<CreateLikeResponse> toggleLike(@PathVariable("reviewId") String reviewId, Authentication authentication) {
        // 인증된 사용자 정보에서 UserDetails를 가져옴
        User userDetails = (User) authentication.getPrincipal();
        String userId = userDetails.getId(); // User의 고유 ID

        CreateLikeResponse response = likeService.toggleLike(userId, reviewId);

        // createdLike가 있으면 생성(201), deletedLike가 있으면 삭제(200)
        if (response.getCreatedLike() != null) {
